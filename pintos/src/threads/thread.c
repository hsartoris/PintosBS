...
thread_unblock (struct thread *t) 
{
	...
	ASSERT (is_thread (t));
	// TODO: check current thread priority against unblocked 
	// thread
...
{
	// TODO: potentially evaluate which thread has highest priority?
	if (list_empty (&ready_list))
		...
		for (e = list_begin(&ready_list);
				...
				e = list_next(e) ){
			struct thread *temp = list_entry(e, 
					struct thread, 
					elem);
